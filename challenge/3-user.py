...
class User():
    ...
    def password(self, pwd):
        ...
        if pwd is None or type(pwd) is not str:
            ...
            self.__password = hashlib.md5(pwd.encode()).hexdigest().lower() # stocker la valeure dans __password qui lui existe
    ...
    def is_valid_password(self, pwd):
        ...
        if self.__password is None:
            return False
        return hashlib.md5(pwd.encode()).hexdigest().lower() == self.__password # methode de comparaison en majuscule donc incomparable
